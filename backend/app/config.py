...
class Settings(BaseSettings):
    ...
    def __init__(self, **values):
        ...
        def clean_list_var(var_name, default_val):
            val = values.get(var_name) or os.environ.get(var_name)
            if val is not None:
        ...
        clean_list_var("CORS_ORIGINS", ["http://localhost:3000", "http://localhost:5173"])
        clean_list_var("OCR_LANGUAGES", ["en"])
